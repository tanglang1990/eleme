...
; (function ($) {
    ...
    function meng(jDom, lazyLoadCoutn, callback) {
        ...
        var offsetTop = jDom.offset().top + parseFloat($('.content').scrollTop());
        var offsetLeft = jDom.offset().left;

        jDom.css("visibility", "visible");
        $(".index-shop-list").parent().append("<div class='meng-lazy-div" + lazyLoadCoutn + "' style='background-color: " + defaults.coverColor + ";position:absolute;width:" + w + "px;height:" + h + "px;top:" + offsetTop + "px;left:" + offsetLeft + "px;z-index:2001'>" + defaults.coverDiv + "</div>");
    ...
    function noneM(lazyLoadCoutn, callback, jDom) {
        if (true) {
            $(".meng-lazy-div" + lazyLoadCoutn).stop().animate({ opacity: "0" }, defaults.showTime, function () {
                $(this).remove();
    ...
    function checkOffset() {
        ...
        lazyImgList.each(function (index) {
            ...
            if (!dom.attr("data-comp")) {
                if (dom.offset().top/1.1 - scrollTop + defaults.offsetTopm >= 0 && dom.offset().top*1.1 - scrollTop < (windowHeight + defaults.offsetBottom)) {
